...
function L = LoadMultipleCorrFunc_v3(varargin);
...
if ischar(varargin{1}),
    ...
    for i = 1: NofFiles,    
        Ls{i}=LoadAutoCorrelation_v6(folderPath, FileNames{i});
    end;
...
for i = 1: NofFiles, 
    ...
    if isfield(Ls{1}, 'v_um_s'),
            L.dt = Ls{1}.dt;
            L.v_um_s = [L.v_um_s; Ls{1}.v_um_s];
            L.median_speeds = [L.median_speeds Ls{1}.median_speed_um_s];
            L.std_speeds = [L.std_speeds Ls{1}.std_speed_um_s];
    end;
    
      if isfield(Ls{1}, 'PhotoDiode'),
        L.PhotoDiode = [L.PhotoDiode; Ls{i}.PhotoDiode];
    end;
        
    if isfield(Ls{1}, 'ImageArray'), % does not seem to be useful
      %  L.ImageArray = [L.ImageArray Ls{i}.ImageArray];
       % L.ImageSize = Ls{1}.ImageSize; 
    end;
end;
...
%L.ImageArray = reshape(L.ImageArray, [L.ImageSize L.NrunsPerformed]);
...
%      L.t_v = L.dt*(1:length(L.v_um_s));
     ...
     L.G0 = robmean(L.AverageCF_CR(j), L.errorCF_CR(j));
     L.Normalized = L.AverageCF_CR./L.G0;
     weights = mean(L.CF_CRgood(j, :));
  
     NormCorrfunc =  L.CF_CRgood./repmat(weights, size( L.CF_CRgood, 1), 1);
     Normalized = mean(NormCorrfunc.*repmat(weights, size( L.CF_CRgood, 1), 1), 2)./mean(repmat(weights, size( L.CF_CRgood, 1), 1), 2);
     NormalizedSq = mean(NormCorrfunc.^2.*repmat(weights, size( L.CF_CRgood, 1), 1), 2)./mean(repmat(weights, size( L.CF_CRgood, 1), 1), 2);
     L.errorNormalized = sqrt(NormalizedSq - Normalized.^2)/sqrt(length(weights));
    % L.errorNormalized = L.errorCF_CR./L.G0;
     L.NormCorrFuncGood = NormCorrfunc;
     L.G0_good = weights;
     
     L.G0All = robmean(L.AverageAllCF_CR(j), L.errorAllCF_CR(j));
     L.NormalizedAll = L.AverageAllCF_CR./L.G0All;
     weights = mean(L.CF_CR(j, :));
  
     NormCorrfunc =  L.CF_CR./repmat(weights, size( L.CF_CR, 1), 1);
     Normalized = mean(NormCorrfunc.*repmat(weights, size( L.CF_CR, 1), 1), 2)./mean(repmat(weights, size( L.CF_CR, 1), 1), 2);
     NormalizedSq = mean(NormCorrfunc.^2.*repmat(weights, size( L.CF_CR, 1), 1), 2)./mean(repmat(weights, size( L.CF_CR, 1), 1), 2);
     L.errorAllNormalized = sqrt(NormalizedSq - Normalized.^2)/sqrt(length(weights));
end;
...
figure;set(gcf, 'units', 'normalized','Position' ,[0.1 0.1 0.8 0.8]); %figure  2
% start changes ilan 2/9/09
% title of the figure
%fname = strcat(folderPath, FileNames{1})
%slashpos=strfind(fname,'\');
%title_name= fname((slashpos(3)+1):end)
% stop changes ilan 2/9/09
semilogx(L.lag(j), L.AverageAllCF_CR(j), L.lag(j), L.AverageCF_CR(j));

% start changes  ilan 2/9/09
% add a title to the 2nd figure

% stop changes figure
%pause
% save pic start changes ilan 2/9/09
%fullFileName = strcat(folderPath, 'aaa.jpg')
%fname = [fullFileName '.jpg'];
%print(gcf, '-djpeg90', '-r70',fullFileName);
% stop changes ilan 2/9/09 
