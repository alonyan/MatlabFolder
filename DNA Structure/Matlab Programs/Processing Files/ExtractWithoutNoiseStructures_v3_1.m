...
%version 3_1: added 'diff3D_speed' model
%
% 14.06.07: added 
% 'FJCreptation_Speed' model
% parameters 
%
% Example
% L48_static = ExtractWithoutNoiseStructures_v3_1(L48_static,  [], 'Average', [1e-3 2e-3], 'Rh6G calibration',  {Rh6G Rh6G 30000 0.002 'All'}, 'Create Model', {'diff3D'})
...
elseif length(j1)==1,
    ...
    if strcmp(y.model.name, 'Parallel-Perpendicular'),
      ...
      if strcmp(y.model.name, 'FJCreptation_Speed'),
        if ~isfield(y.model, 'addParameters'),
            error('Have to supply characteristic time!'),
        end;
        %velocity = 3;
...
y
y.displacement=ExtractRsqModel_v2_1(L, y.model, FitType, range, CorrTypeToDisplacement, 'wXYsq', y.SamplingVolume.wXYsq);
y.withoutAdditiveNoise=ExtractRsqModel_v2_1(LwoNoise, y.model, FitType, range, CorrTypeToDisplacement,'wXYsq', y.SamplingVolume.wXYsq);
if exist('corrWoCorrNoise')==1, 
    y.withoutCorrNoise=ExtractRsqModel_v2_1(corrWoCorrNoise, y.model, FitType, range, CorrTypeToDisplacement, 'wXYsq', y.SamplingVolume.wXYsq);
end;
