function [A, Pict]=OpenAndSaveLVimage(varargin)  %(path, filename)
path = varargin{1};
filename=varargin{2};
fullFileName = strcat(path, filename)

%fullFileName = 'G:\Ilan\Experiments\200809\his_mind_noatp_12'
...
colormap('gray') ;imagesc(Pict, [min(Pict(:))-1, max(Pict(:))/3]); axis image; title(title_name)
