%% Processing  data of 131008: semidilute solutions of linear DNA
...
DiluteFieldName = 'L3_2_v4111p13_1';
...
data131008.field500.q = data131008.DNA500.q;
data131008.field500.fq = data131008.DNA500.fq./SB500.Sq;
data131008.field500.fq_int = data131008.DNA500.fq_int./SB500.Sq;
data131008.field500.errorFq = data131008.DNA500.errorFq./SB500.Sq;
semilogy(data131008.field500.q.^2, [data131008.field500.fq data131008.field500.fq_int]); 
%axis([0 1 1e-3 1.2]);
figure(gcf)
...
showRange =20;
NormRange = [1 4]*1e-3;
B1.lag = data131008.DNA500.lag;
B1.vt_um = [data131008.DNA500.vt_um  data131008.DNA500.vt_um];
B1.Normalized = [ones(size(data131008.DNA500.Corrected))   data131008.DNA500.Corrected];
B1.errorNormalized = [data131008.DNA500.errorNormalized data131008.DNA500.errorNormalized];

FigHdl = FitField_GUI_twoCF(B1, minVt, dynRange, showRange, lowestG, 30, NormRange*1e3)
%% 3) Extract field 
B1 = guidata(FigHdl);
close(FigHdl);
data131008.field500.wS = B1.field.paramGauss(2)
b = 0.1;
L = 1.5*500*0.34e-3
Rg = sqrt(L*b/6 -b^2/4 + b^3./(4*L) - b^4./(8*L.^2).*(1 - exp(-2*L/b)))
data131008.field500.wXY = sqrt(data131008.field500.wS^2 - 2/3*Rg^2)
...
for i=1:length(fnamesData),
    ...
    S.Sq = S.fq./data131008.field500.fq;
    S.Sq_int =S.fq_int./data131008.field500.fq_int;
    S.errorSq = sqrt((S.errorFq./S.fq_int).^2 +(data131008.field500.errorFq./data131008.field500.fq_int).^2).*S.Sq_int;
...
data131008.ksi_um = data131008.ksi*data131008.SQ0.fitDebye(1)/sqrt(2);
...
% Converting ksi into dimensional ksi (in um)
data131008.ksi_th_um = data131008.ksi_th*data131008.SQ0.fitDebye(1)/sqrt(2);

%% Combine and fit data
load process051008 data230908_proc0510 data250908_proc0510
load process201008 data131008
conc = [data230908_proc0510.conc data250908_proc0510.conc data131008.conc];
ksi_um =  [data230908_proc0510.ksi_um data250908_proc0510.ksi_um data131008.ksi_um];
