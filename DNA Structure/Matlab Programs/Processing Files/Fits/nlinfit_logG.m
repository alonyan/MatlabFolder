function [beta,r,J,Sigma,mse] = nlinfit_logG(X,y,model,beta, varargin)
...
%   $Revision: 2.22.2.11.2.1 $  $Date: 2007/07/19 18:17:55 $
...
j1=find(strcmp(varargin, 'Options'));
if length(j1)>1,
    error('Too many Options inputs!')
elseif length(j1)==1,
    options = varargin{j1+1};
    options = statset(statset('nlinfit'), options);
else %default
    options = statset('nlinfit');
end;

j1=find(strcmp(varargin, 'ErrorWeights'));
if length(j1)>1,
    error('Too many ErrorWeigths inputs!')
elseif length(j1)==1,
        errorWeight = varargin{j1+1};
    if length(errorWeight)~=length(y)
        error('the length of weights vector should equal to that of Y vector.')
    end;
else
    errorWeight = ones(size(y));
end;

j1=find(strcmp(varargin, 'Parameters'));
if length(j1)>1,
...
r = y(:) - yfit(:);
r =  unwrap_logG(r, errorWeight, unwrapTag);
r(nans) = [];
n = numel(r);
p = numel(beta);
sse = r'*r;
...
if strcmp(options.Robust,'off')
    ...
    [beta,J,lsiter,cause] = LMfit(X,y, model,beta,options,verbose,maxiter, errorWeight, param, unwrapTag);
else
    ...
    % limit.
    [beta_ls,J] = LMfit(X,y, model,beta,options,0,maxiter, errorWeight, param, unwrapTag);
    if isempty(param),
        res = y - model(beta_ls,X);
    else
        res = y - model(beta_ls,X, param);
    end;
    ...
    [beta,J,sig,cause] = nlrobustfit(X,y,beta,model,J,ols_s,options,verbose,maxiter, errorWeight, param, unwrapTag);  
end;
...
%----------------------------------------------------------------------
function  [beta,J,iter,cause] = LMfit(X,y, model,beta,options,verbose,maxiter, errorWeight, param, unwrapTag) 
...
while iter < maxiter
    ...
    J = getjacobian(beta,fdiffstep,model,X,yfit,nans, errorWeight, param);
    ...
    step = Jplus \ rplus;
    beta(:) = beta(:) + step;
    %confine beta to real positive values:
    beta = real(beta);
   % beta(beta < 0) = 0;
...
%--------------------------------------------------------------------------
function [beta,J,sig,cause]=nlrobustfit(x,y,beta,model,J,ols_s,options,verbose,maxiter, errorWeight, param, unwrapTag)
...
while maxiter>0
    ...
    if isempty(param),
        ...
        modelw = @(b,x) sqrt(w).*model(b,x, param);
    end;
    [beta,J1,lsiter,cause] = LMfit(x,yw,modelw,beta0,options,0,maxiter, errorWeight, param, unwrapTag); % 6th arg always silences display
...
fdiffstep = options.DerivStep;
J = getjacobian(beta,fdiffstep,model,x,yfit,~ok, errorWeight, param);
...
% ---------------------- Jacobian
function J = getjacobian(beta,fdiffstep,model,X,yfit,nans, errorWeight, param)
p = length(beta);
...
for k = 1:p
    ...
    if isempty(param),
        ...
        yplus = model(beta+delta,X, param);
    end;
    dy = (yplus(:) - yfit(:))./errorWeight;
    dy(nans) = [];
    J(:,k) = dy/delta(k);
    delta(k) = 0;
end
...
function rnew =  unwrap_logG(r, errorWeight, unwrapTag)
rnew = r;
% j1 = (imag(r) > pi);
% j2 = (imag(r) < -pi);
% rnew (j1) = r(j1) - 2i*pi;
% rnew (j2) = r(j2) + 2i*pi;
%rnew = ([real(rnew) imag(rnew)]./[real(errorWeight) imag(errorWeight)]).^2;
%rnew = sqrt(sum(rnew, 2));
if strcmp(unwrapTag, 'complex')
    lenr = length(r);
    imagr = r((lenr/2+1):lenr);
    j1 = find(imagr > pi);
    j2 = find(imagr < -pi);
    if ~isempty(j1),
        rnew (lenr/2+j1) = imagr(j1) - 2*pi;
    end;
    if ~isempty(j2),
        rnew (lenr/2+j2) = imagr(j2) + 2*pi;
    end;
%[r rnew];
end;
%errorWeight
rnew = rnew./errorWeight;
