function [beta, yfit,cfg] = RegularizedFit_v1(t,y, FitFunc, beta0, param,  alpha,varargin)
%     inspired by rilt  program that does Regularized Inverse Laplace Transform
% 
%  y is a function of t
%  FitFunc is a pointer to the fitting function
% beta0  is a set of initial parameters of the fit
% param  are fixed parameters in the FitFunc
% alpha is a regularization parameter
%
% Varargin parameters
% 'Plot Type':   @semilogx, @loglog etc
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
beta0 = beta0(:); % must be column
param = param(:); % must be column
%y = y(:); % must be column
%t = t(:); % must be column

j1=find(strcmp(varargin, 'Plot Type'));
if length(j1)>1,
    error('Too many Plot Type inputs!')
elseif length(j1)==1,
    plot_type = varargin{j1+1};
else
    plot_type = @plot;
end;
...
j1=find(strcmp(varargin, 'MaxSearch'));
if length(j1)>1,
    error('Too many MaxSearch inputs!')
elseif length(j1)==1,
    maxsearch = varargin{j1+1};
else
    maxsearch = 1000;
end;

j1=find(strcmp(varargin, 'Constraints'));
if length(j1)>1,
    error('Too many Constraints inputs!')
elseif length(j1)==1,
    constraints = varargin{j1+1};
else
    constraints = {'beta > 0','zero_at_the_extremes'};
end;

j1=find(strcmp(varargin, 'Errors'));
if length(j1)>1,
    error('Too many Errors inputs!')
elseif length(j1)==1,
    w = 1./varargin{j1+1}.^2; % weights
else
    w = ones(size(y));
end;

R = zeros(length(beta0)-2,length(beta0));
...
for k = 1:maxsearch,
    ...
    [beta, msdfit] = fminsearch(@msd,beta0, options, FitFunc, param, t, y, alpha,R, w, constraints);
    ...
    if ismember('beta > 0',constraints)
        beta = abs(beta);
    end
    %--------------------
    beta0 = beta; % for the next step
...
% ### SUBS #####################################################
function out = msd(beta, FitFunc, param, t, y, alpha, R, w, constraints)
% msd: The mean square deviation; this is the function
% that has to be minimized by fminsearch
...
if ismember('beta > 0',constraints)
    beta = abs(beta); % must be g(i)>=0 for each i
end
%r = diff(diff(beta(1:end))); % second derivative of g
r = diff(beta, 2); % second derivative of beta
%yfit = A*g;
yfit = FitFunc(beta, t, param);
