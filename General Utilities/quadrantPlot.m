function [Cent, Lbls] = quadrantPlot(x1,x2, varargin)
...
cla
h = PlotColorCoded_v1(x1,x2,1:numel(x1), 1:numel(x1), 25,'np', min(10000,numel(x1)));
...
% +/+ +/- -/+ -/-
Lbls = ((x1>=Cent(1)).*(x2>=Cent(2)))+2*((x1>=Cent(1)).*(x2<Cent(2)))+3*((x1<Cent(1)).*(x2>=Cent(2)))+4*((x1<Cent(1)).*(x2<Cent(2)));
hold on
