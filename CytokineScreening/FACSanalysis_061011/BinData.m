function [binnedData, Xbins, Nbins] = BinData(Data, varargin)
...
%Xbins            
        j1=find(strcmp(varargin, 'Xbins'));
            ...
            elseif isempty(j1),
                RangeMin = min(Data(1).data(:,ch1));
                for i = 2:length(Data)
                    RangeMin = min(RangeMin, min(Data(i).data(:,ch1)));
                end;
                                RangeMax = max(Data(1).data(:,ch1));
                for i = 2:length(Data)
                    RangeMax = max(RangeMax, max(Data(i).data(:,ch1)));
                end;
                DataHist = HistAsinh3(Data, 'Tag' ,Tag,'LinearRange', LinearRange, 'Nbins', Nbins+1, 'RangeMin' , RangeMin,'RangeMax', RangeMax);
                Xbins = DataHist.(Tag).Xtrue;
            end;
