function [procData, LinearRange] = HistAsinh3beta(Data, varargin)
...
%     end;
    ...
    if strcmp(dataType, 'numeric2D'),
        ...
        YvsX.(['Hnorm_' Tag{1}]) = Hnorm./repmat(sum(Hnorm, 1), size(Hnorm, 1), 1);
        YvsX.(['errHnorm_' Tag{1}]) = errHnorm./repmat(sum(Hnorm, 1), size(Hnorm, 1), 1);
        YvsX.(['Hnorm_' Tag{2}]) = Hnorm./repmat(sum(Hnorm, 2), 1, size(Hnorm, 2));
        YvsX.(['errHnorm_' Tag{2}]) = errHnorm./repmat(sum(Hnorm, 2), 1, size(Hnorm, 2));       
