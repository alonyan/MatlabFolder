function [CGData, CompensationMatrix, WorkSp, groupInd, gateInd] = LoadFACSfromFlowJoWorkspace_v6( fpath, varargin)
...
%               the program, but keep the basic presentation the same as
%               before
%
% version 6 - Feb. 2014 Presenting loaded gates all on the same page;
%               different data presented on different pages.
%           Constants at the beginning of the file:
%           Decrease MaxPointsToPlot to speed up the plotting procedures
%           Increase Nbins for smoother color coded density presentation
...
FigureWindPos = [0.02 0.16 0.96 0.72];
    ...
    if length(workspace_file) == 1,
        WorkSp = ReadFlowJoXMLfileToObjects_v8([fpath filesep workspace_file.name]);
    elseif length(workspace_file) == 0,
        he = errordlg('XML workspace file was not found. Please point to the file');
        WorkSp = ReadFlowJoXMLfileToObjects_v8;
        close(he);
...
%     ListOfFCSFiles = {ListOfFiles.name};
    ...
    if isfield(WorkSp, 'CompMatrix'),  %patching the absence of Compensation matrix: assume identity matrix
    ...
    else % no compensation matrix: assume no need
        warning('No compensation matrix found: using identity matrix')
    end;

    %load the file with the largest number of events;
    %show the application of gates
    %get the channel names
    
    GroupSamples = WorkSp.Samples(WorkSp.sampleID2sampleInd(WorkSp.Group(groupInd).SampleID));
    [~, I] = max([GroupSamples.count]);
    ...
    tempname = [fpath filesep filename];
        if exist(tempname, 'file'),
            [~, fcshdr] = fca_readfcs_v2(tempname);
...
%     WorkSp.CompMatrix.ChannelNames
    ...
    else % patching the absence of Compensation matrix
        ListofChannelsWithLabels = ListOfChannels;
        CombinedListofChannels = ListOfChannels;
        CombinedListofChannelsComp = cell(0);
        NcompChann = 0;
        Jcomp = [];
        for NChan = 1: length({fcshdr.par.tag})
            if ~isempty(fcshdr.par(NChan).tag)
                ListofChannelsWithLabels{NChan} = [ ListOfChannels{NChan} ': ' fcshdr.par(NChan).tag]
            end
        end
    end;
    ...
    ListOfNames = {GroupSamples.fname};
    ...
    for j = 1:length(ListOfFCSFiles),
        HF(j) = figure('Name', ListOfNames{j}, 'Units', 'Normalized', 'Position', FigureWindPos);
        ...
        if isfield(WorkSp, 'CompMatrix'),
            fcsdata(:, Jcomp)=fcsdata(:, Jcomp)/CompensationMatrix;
        end;
        ...
        NoAxesInRow = round(sqrt(1.5*length(gateInd)));
        NoAxesInCol = ceil(length(gateInd)/NoAxesInRow);
        
        for i=1:length(gateInd),
            subplot(NoAxesInCol, NoAxesInRow, i);
            ...
            if strcmp(gate.selectionType, 'PolyRect'),
               ...
               % if (Demonstration)|strcmp(GatesToLoad, 'SampleGates'),
                   ...
                   LogXscale = ~(any(strfind(gate.xAxis, 'FSC')) || any(strfind(gate.xAxis, 'SSC')) || strcmp(gate.xAxis, 'time'));
                   LogYscale = ~(any(strfind(gate.yAxis, 'FSC')) || any(strfind(gate.yAxis, 'SSC')));
                   h = ScatterPlotFACS_v1(Xdata, Ydata, 'LogXscale', LogXscale, 'LogYscale', LogYscale,...
                       'PlotGate', [Xgate Ygate], 'MaxPointsToPlot', MaxPointsToPlot, 'Nbins', Nbins);
                   

                   xlabel(gate.xAxis, 'FontSize', LabelFontSize);
                   ylabel(gate.yAxis, 'FontSize', LabelFontSize);
                   title(gate.name, 'FontSize', TitleFontSize);
       %            pause;
            ...
            elseif (strcmp(gate.selectionType, 'Range') & isempty(yInd)), %Histogram plot
                ...
                if Demonstration, 
                   ...
                   h = ScatterPlotFACS_v1(Xdata, [], 'LogXscale', LogXscale,'PlotGate', Xgate,...
                       'MaxPointsToPlot', MaxPointsToPlot, 'Nbins', HistBins);
  ...
  %                 pause;
        ...
        close all;
        ListOfFCSFiles{j}
        CGData(j).name = ListOfNames{j};
        CGData(j).fname = ListOfFCSFiles{j};
