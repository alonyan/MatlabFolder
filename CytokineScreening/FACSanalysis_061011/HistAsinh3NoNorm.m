function [procData, LinearRange] = HistAsinh3NoNorm(Data, varargin)
       ...
       if strfind(dataType, '1D'),
           if strfind(dataType, 'numeric'),
               ...
               for i = 1:length(TagData),
                   tempData = [tempData; TagData(i).(TagName)];
               end;
           end;
...
HnoNorm = [];
        ...
        for i=1:length(TagData),
            Htemp = hist(asinh(TagData(i).(TagName)/LinearRange), X);
            Htemp = Htemp(:);
...
%            HnormTemp = Htemp./dXnorm;
            ...
            meanD = [meanD mean(TagData(i).(TagName))];
            medianD = [medianD median(TagData(i).(TagName))];
            geomeanD = [geomeanD exp(mean(log(TagData(i).(TagName))))];
            CVD = [CVD std(TagData(i).(TagName))/meanD(i)];
            
            if  cellNoTemp > 10,
                 wYtrue = sort(TagData(i).(TagName), 'descend');
        ...
        procData.(TagName).H = H;
        procData.(TagName).HnoNorm = HnoNorm;
        procData.(TagName).errH = errH;
        procData.(TagName).Hnorm = Hnorm;
        procData.(TagName).errHnorm = errHnorm;
        procData.(TagName).X = X;
        procData.(TagName).Xtrue = Xtrue; 
        procData.(TagName).meanD = meanD;
        procData.(TagName).medianD = medianD;
        procData.(TagName).geomeanD = geomeanD;
        procData.(TagName).CVD = CVD; 
        procData.(TagName).LinearRange = LinearRange;
        procData.(TagName).cellNo = cellNo;
        
        procData.(TagName).cdfvect = cdfvect;
        procData.(TagName).cdfval = cdfval;
