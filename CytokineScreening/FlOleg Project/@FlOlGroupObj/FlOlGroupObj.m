classdef FlOlGroupObj < handle
    ...
    methods
        function obj = FlOlGroupObj(varargin)
            if (length(varargin)==1), % expect a structure with named fields
                if isstruct(varargin{1}),
                    GroupStruc = varargin{1};
                    fldnames = fieldnames(GroupStruc);
                    for i=1:length(fldnames),
                       if ismember(fldnames{i}, properties('FlOlGroupObj')),
                           obj.(fldnames{i}) = GroupStruc.(fldnames{i}); 
                       else
                           warning(['A field ' fldnames{i} ...
                               ' does not have a counterpart in the Group Object!!!'])
                       end;
        ...
        function obj = insertPopulation(obj, newGate)
            if (newGate.ParentHndl == obj), % if the population is derived directly from the Group root
