   function [WorkSpNoPointers, FlOlegWorkSpace, WorkSp, theTree, SampleNodeObj, GroupsObj] = ReadFlowJoXMLfileToObjects_v7(varargin)
 ...
 %get sample information
    ...
    for i=0:(SampleNodeObj.getLength-1), 
       ...
       % Samples(i+1).fname = char(SampleNodeObj.item(i).getAttribute('name'));
        Samples(i+1).name = char(SampleNodeObj.item(i).getAttribute('name'));
        ...
        if i==0, 
            ...
            for j=0:(KeywordsObj.getLength - 1),
                ...
                elseif strcmp(KeywordsObj.item(j).getAttribute('name'), '$FIL'),
                    Jfilename = j;
            ...
            if exist('Jfilename'),              
                if strcmp(KeywordsObj.item(Jfilename).getAttribute('name'), '$FIL'), %use the found "Plate Name" keyword
                        Samples(i+1).fname = char(KeywordsObj.item(Jfilename).getAttribute('value'));
                else %look for a new one
                    'should not be here'
                    for j=0:(KeywordsObj.getLength - 1),
                        if strcmp(KeywordsObj.item(j).getAttribute('name'), '$FIL'),
                            Jfilename = j;
                            Samples(i+1).fname = char(KeywordsObj.item(j).getAttribute('value'));
                            break;
                        end;
