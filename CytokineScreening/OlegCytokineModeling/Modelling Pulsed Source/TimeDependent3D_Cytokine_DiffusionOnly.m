function S = TimeDependent3D_Cytokine_DiffusionOnly(t, x, J, varargin)
...
% Distance is measured in the units of cell radius R,
% and the total flux in fact (the rate of cytokine
% secretion)/(4*pi*R*D*EC50)=c0 in EC50s (to calculate response)
    ...
    function [a,f,s] = pdex1pde(X,t,u,DuDx) % The equation
        a = 1;
        f = DuDx;
        s = 0; %-1./(1+u.^(-h)); % the consumption term
    end
    % --------------------------------------------------------------
    function u0 = pdex1ic(X) % zero initial conditions
        u0 = 0;
    end
