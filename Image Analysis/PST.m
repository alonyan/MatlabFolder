...
% Image_orig_f=Image_orig_f.*fft2(exp(-(RHO/sqrt(sigma)).^2));
% Image_orig_filtered=fftshift(real(ifft2((Image_orig_f))));
...
temp=fft2(Image_orig_filtered).*fftshift(exp(-1j*pi*PST_Kernel));
