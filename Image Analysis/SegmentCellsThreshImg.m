function [L, voronoiCells] = SegmentCellsThreshImg(img, varargin)
...
GMimg = GradMag_v1(imgSmooth); %use DAPI for segmentation
...
hThresh = ParseInputs('hthresh', 0.002, varargin);
...
if ~isstruct(voronoiCells)&&isempty(voronoiCells)
    ...
    Mask = imgSmooth>triThreshImg(imgSmooth);
    
    
    % Threshold
    img_hmax = imhmax(imgSmooth.*double(Mask),hThresh); %threshold
...
gradmag2 = imimposemin(GMimg(2:end-1, 2:end-1), imgBW | RegionBounds | ~Mask(2:end-1, 2:end-1));
gradmag2 = imhmax(gradmag2,gmThresh);
