function [out PST_Kernel]= PST3D(I,AspectRatio,handles,Morph_flag)
...
L=0.5;
x=linspace(-L*AspectRatio(1),L*AspectRatio(1),Image_orig_size(1));
y=linspace(-L*AspectRatio(2),L*AspectRatio(2),Image_orig_size(2));
z=linspace(-L*AspectRatio(3),L*AspectRatio(3),Image_orig_size(3));
[X,Y,Z]=ndgrid(x,y,z);
...
% 
% Z_step=z(2)-z(1);
% fz=linspace(-0.5/Z_step,0.5/Z_step,length(z));
% fz_step=fz(2)-fz(1);
...
if handles.LPF
    sigma=handles.LPF;
    h = fspecial('gaussian', round(5*sigma),sigma);
    Image_orig_filtered = imfilter(I,h,'replicate');
else
    Image_orig_filtered = I;
end
...
PST_Kernel=PST_Kernel/max(PST_Kernel(:))*handles.Phase_strength;

% Apply the PST Kernel
temp=(fftn(Image_orig_filtered)).*fftshift(fftshift(exp(1j*pi*PST_Kernel)),3);
...
if Morph_flag ==0
    ...
    features(PHI_features>handles.Thresh_max)=1;
    features(PHI_features<handles.Thresh_min)=1;  % because output phase has both positive and negative values
    features(I<(max(I(:))/20))=0; % ignore the features in the very dark areas of the image
