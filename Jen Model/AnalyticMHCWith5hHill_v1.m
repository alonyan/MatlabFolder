function y = AnalyticMHCWith5hHill_v1(param,t)
...
sigma2=7*60*60;              %avg protein halflife (sec) (my data)
beta2=(log(2))/sigma2; 
...
y = (((l*k*pSTAT1_ss_sc/(pSTAT1_ss_sc+Kd))/(beta2-beta1)).*((-exp(-beta1*t))/beta1+(exp(-beta2*t))/beta2+((beta2-beta1))/(beta1*beta2))).*(heaviside(t)-heaviside(t-tau))+...
    (((l*k*pSTAT1_ss_sc/(pSTAT1_ss_sc+Kd))/(beta2-beta1)).*((exp(beta1*tau)-1)*(exp(-beta1*t))/beta1-(exp(beta2*tau)-1)*(exp(-beta2*t))/beta2)).*heaviside(t-tau); %protein
