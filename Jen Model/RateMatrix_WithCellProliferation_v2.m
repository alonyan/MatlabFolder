function dx=RateMatrix_WithCellProliferation_v2(t, x,parameters)
...
dx(2) = -HSkon*x(2)*x(1)+HSkoff*x(3)+k_cellcycle*(x(2)+x(3)); %HS

dx(3) = HSkon*x(2)*x(1)-HSkoff*x(3); %gamma HS

dx(4) = -kon*x(4)*x(1)+koff*x(5)+k_cellcycle*(x(4)+x(5))+kend*x(5); %gR

dx(5) = +kon*x(4)*x(1)-koff*x(5)-kend*x(5);%gRC  

%dx(6)= k_cellcycle*log(cellcapacity/x(6))*x(6); %Number of Cells modeled with Gompertz law

dx(1) = -dx(3)-dx(5); %free gamma
