function y = BadFunc(param,t,varargin)
...
pSTAT1_ss_sc=(pSTAT1_ss*nA)/10e12;
    ...
    if tdiv
        ...
        if tau
            y = (((l*k*pSTAT1_ss_sc)/(beta2-beta1)).*((exp(beta1*tau)-1)*(exp(-beta1*t))/beta1-(exp(beta2*tau)-1)*(exp(-beta2*t))/beta2)).*heaviside(t-tau); %protein
        else
            y = (((l*k*pSTAT1_ss_sc)/(beta2-beta1)).*((-exp(-beta1*t))/beta1+(exp(-beta2*t))/beta2+((beta2-beta1))/(beta1*beta2)));
        end;
    end;
