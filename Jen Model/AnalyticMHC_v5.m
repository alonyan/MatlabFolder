...
pSTAT1_ss_sc=(pSTAT1_ss*nA)/10e12;
    ...
    if tdiv  %if a division time is supplied
       tStart=0:tdiv/100:tdiv;
       for Ind = 1:length(tStart)
        gen = (floor((t+tStart(Ind))./tdiv));
        ...
        y(Ind,:) = (((l*k*pSTAT1_ss_sc)/(beta2-beta1)).*((-exp(-beta1*t))/beta1+(exp(-beta2*t))/beta2+((beta2-beta1))/(beta1*beta2))).*(heaviside(t)-heaviside(t-tau))+...
        ...
        y(Ind,:) = l*(An0.*exp(-beta1*t))./(beta2-beta1)+k*l*pSTAT1_ss_sc./(beta1*beta2)+Bn0.*exp(-beta2*t);
        end
       end
       y = mean(y,1); 
        ...
        if tau %if a cytokine stop time is supplied
            ...
            (((l*k*pSTAT1_ss_sc)/(beta2-beta1)).*((exp(beta1*tau)-1)*(exp(-beta1*t))/beta1-(exp(beta2*tau)-1)*(exp(-beta2*t))/beta2)).*heaviside(t-tau)+((k-k0)*l*c)./(k0*beta1*beta2); %protein
        else %if a cytokine stop time is not supplied
            y = (((l*k*pSTAT1_ss_sc)/(beta2-beta1)).*((-exp(-beta1*t))/beta1+(exp(-beta2*t))/beta2+((beta2-beta1))/(beta1*beta2)))+((k-k0)*l*c)./(k0*beta1*beta2);
        end;
    end;
