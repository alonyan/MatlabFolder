...
tdiv = 60*60*16; %division time
gen = (floor(t./tdiv));

A00 = -(k*pSTAT1_ss_sc(n))./(beta1);
B00 = l*k*pSTAT1_ss_sc(n)./(beta2*(beta2-beta1))
...
B = l*(An0.*exp(-beta1*t))./(beta2-beta1)+k*l*pSTAT1_ss_sc(n)./(beta1*beta2)+Bn0.*exp(-beta2*t)+(l*c)/(beta1*beta2);
...
%% quasi steady state approx with signal stop at tau, taking pSTAT1 at its s.s. value. For derivation see writeup. Adding splitting every 16h
 t0=0;               %Initial time, (minutes)
 tstop=60*60*100;          %Finish time, 
 dt=1;
 t = t0:dt:tstop;
...
tdiv = 60*60*25; %division time
gen = (floor(t./tdiv));

A00 = ((k*pSTAT1_ss_sc(n))./(beta1)).*(exp(beta1*tau)-1);
B00 = -(l*k*pSTAT1_ss_sc(n)./(beta2*(beta2-beta1))).*(exp(beta2*tau)-1);
...
B = (((l*k*pSTAT1_ss_sc(n))/(beta2-beta1)).*((-exp(-beta1*t))/beta1+(exp(-beta2*t))/beta2+((beta2-beta1))/(beta1*beta2))+(l*c)/(beta1*beta2)).*(heaviside(t)-heaviside(t-tau))+...
    ((l*An0./(beta2-beta1)).*exp(-beta1*t)+Bn0.*exp(-beta2*t)+(l*c)/(beta1*beta2)).*heaviside(t-tau); %protein
