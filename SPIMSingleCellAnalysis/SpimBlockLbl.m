    ...
    methods
        ...
        function Blocklbl = SpimBlockLbl(fpath, partitionInfo,timepoint,tile,NuclearChannel)%constructor
            ...
            if ~isempty(Data)
                ...
                ThreshVal = exp(meanThresh(log(datasample(Data(:), 500000))));
        ...
        function stkshow(W,varargin)
            MD=ParseInputs('SpimMD', SpimMetadata(W.pth), varargin);
            Ch = ParseInputs('Channel',W.NuclearChannel,varargin);
            Data = stkread(MD,W.Frame,W.tile,Ch,varargin{:});
            stkshow(Data)
        end
        ...
        function scattershow(W,varargin)
            ...
            refTile = ParseInputs('refTile', W.tile, varargin);
            range = ParseInputs('range', 1:W.num, varargin);
            showData = ParseInputs('showData', true, varargin);
            
            RChannel=zeros(W.ImageDims,'single');
            GChannel=zeros(W.ImageDims,'single');
            BChannel=zeros(W.ImageDims,'single');
            ...
            indCh = find(strcmp(W.channelNames,Ch));
            Ints = W.Int90Prctile{indCh};
            
            climits = [prctile(Ints,5), prctile(Ints,95)];
            Ints = (Ints-climits(1))/(climits(2)-climits(1));
            
            cmap = viridis(1024)*0.25;%scale colormap so that data and centroids are all visible
            
            CtoShow = interp1(linspace(0,1,1024), 1:1024,Ints,'nearest','extrap');
            
            tForms = MD.partitionInfo.tForms(find((MD.partitionInfo.timepoint==refTP).*(MD.partitionInfo.tile==refTile).*(MD.partitionInfo.channel==1)));
            ChT = eye(4);
            ...
            for i=1:numel(range)
                ...
                if ~isnan(Cent)
                    %apply inverse transform to center in tile world
                    Cent = ChT0\[Cent 1]';
                    Cent = round(Cent(1:3))';
                    indexToChange = sub2ind(W.ImageDims,min(max(Cent(:,2),1),W.ImageDims(1)),min(max(Cent(:,1),1),W.ImageDims(2)),min(max(Cent(:,3),1),W.ImageDims(3)));
            ...
            se = strel('sphere',3);
            RChannel = imdilate(RChannel,se);
            GChannel = imdilate(GChannel,se);
            BChannel = imdilate(BChannel,se);   
            
            if showData
                %W.stkshow(varargin{:})
                Data = stkread(MD,W.Frame,W.tile,Ch,varargin{:});
                RChannel(RChannel==0) = Data(RChannel==0);
                GChannel(GChannel==0) = Data(GChannel==0);
                BChannel(BChannel==0) = Data(BChannel==0);
                %                 for i=1:W.ImageDims(3)
                %                     MIJ.selectWindow('Data');
                %                     MIJ.setSlice(i);
                %                     MIJ.selectWindow('Composite');
                %                     MIJ.setSlice(i);
                %                     MIJ.selectWindow('Data');
                %                     MIJ.run('Add Image...', 'image=[Composite] x=0 y=0 opacity=100 zero');
                %                 end                
            end
            stkshow(RChannel,'title','R');
            ij.IJ.setMinAndMax(0, 10000)
            stkshow(GChannel,'title','G');
            ij.IJ.setMinAndMax(0, 10000)
            stkshow(BChannel,'title','B');
            ij.IJ.setMinAndMax(0, 10000)
            MIJ.run('Merge Channels...', 'c1=R c2=G c3=B create')
            %MIJ.selectWindow('Composite');
            %MIJ.run('Stack to RGB', 'slices');            
