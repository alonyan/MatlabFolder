...
if strcmpi(extName, '.xml')    % get meta structure
    ...
    img_info('Colors') = length(unique(cellfun(@(x) x.attributes.channel.Text,metaStr.(datasetName).SequenceDescription.ViewSetups.ViewSetup)));
...
%     img_info('Depth') = xyzVal(3);
    ...
    t2 = str2double(strsplit(metaStr.(datasetName).SequenceDescription.Timepoints.integerpattern.Text,','))+1;
    t2 = t2(end)
    %img_info('Time') = [t1 t2];
    img_info('Time') = t2;
