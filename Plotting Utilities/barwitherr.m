...
ErrorWidth = 1;
    j1=find(strcmp(varargin, 'ErrorWidth'));
    if length(j1)>1,
        error('Too many InitParam inputs!')
    elseif length(j1)==1,
        ErrorWidth = varargin{j1+1};
        varargin(j1) = [];
        varargin(j1) = [];
    end;
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        set(hErrorbar(col), 'marker', 'none','LineWidth',ErrorWidth)
    end
else
    if verLessThan('matlab', '8.4')
        % Original graphics:
    ...
    set(hErrorbar, 'marker', 'none','LineWidth',ErrorWidth)
end
