function [h] = PicComment_v1(varargin) 
...
lngth = 50; % default scalebar length in kilometers
string = 'data';
...
% Check for user-declared location: 
tmp = strncmpi(varargin,'str',3); 
if any(tmp)
    string = varargin{find(tmp)+1}; 
    tmp(find(tmp)+1)=1; 
    varargin = varargin(~tmp); 
end
...
switch lower(orientation)
    ...
    switch lower(location)
        ...
        case {'northeast','ne'}
            ...
            y2 = y1;  
                    case {'northeast1','ne1'}
            x1 = .52*(xlim(2)-xlim(1))+xlim(1); 
            x2 = x1-lngth/mppx; 
            y1 = .995*(ylim(2)-ylim(1))+ylim(1); 
            y2 = y1;  
                                case {'northeast2','ne2'}
            x1 = .35*(xlim(2)-xlim(1))+xlim(1); 
            x2 = x1-lngth/mppx; 
            y1 = .95*(ylim(2)-ylim(1))+ylim(1); 
            y2 = y1;  
...
h(2) = text(mean([x1 x2]),mean([y1 y2]),string,...
    'horizontalalignment','left',...
    'verticalalignment','bottom','Color', colour, 'FontSize', 24);
